...
#define HTTP_SERVER_IP "api.thingspeak.com"

//! Set HTTP hostname
#define HTTP_HOSTNAME "api.thingspeak.com"
...
//! HTTP resource name
#define HTTP_URL "/update?api_key=YOUR_API_KEY&field1=0" //Your API Key
...
char *http_hostname = "api.thingspeak.com";
char httpurl[80];
...
void app_init(const void *unused)
{
  ...
  //This initialisation is required to do clock and dma assignments to teh I2C lines and pins
  si70xx_example_init();
  mpu6050_init();
  //comment out the following initialisatiion functions if you aren't using vibration and flex sensors
...
sl_status_t http_client_application(void)
{
  ...
  do {
      status = sl_net_dns_resolve_hostname(http_hostname, DNS_TIMEOUT, SL_NET_DNS_TYPE_IPV4, &dns_query_rsp);
      dns_retry_count--;
...
//  printf("\r\nHTTP Get request init success\r\n");
  ...
  while(1){
    ...
    CLEAN_HTTP_CLIENT_IF_FAILED(status, &client_handle, HTTP_SYNC_RESPONSE);
    printf("\r\nHTTP Get request init success\r\n");
    si70xx_example_process_action(&posture);
    //comment out the following 3 lines if you wish not to use the vibration and flex sensors
    getflexvalue(&flexvsensval);
    vibrstate = get_vibration_sense_status();
    DEBUGOUT("The flex sensor value is %f and teh state of vibration sensor is %d",flexvsensval,vibrstate);
    DEBUGOUT("\nThe posture is %d",posture);

    sprintf(httpurl,"/update?api_key=YOUR_API_KEY&field1=%d",posture); //YOUR API key
    client_request.resource = (uint8_t*)httpurl;

    //! Send HTTP GET request
    status = sl_http_client_send_request(&client_handle, &client_request);
    if (status == SL_STATUS_IN_PROGRESS) {
      while (!http_rsp_received) {  // Wait for response flag
          delay(50);
      }
      DEBUGOUT("\nResponse code is %d",http_rsp_received);
      http_rsp_received = 0;  // Reset flag
    }
    else
       DEBUGOUT("\nThe error code is 0x%lX",status);
    while(temp1--)
      delay(50);
  }
