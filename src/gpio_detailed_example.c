...
static sl_si91x_gpio_pin_config_t sl_gpio_pin_config = { { SL_SI91X_GPIO_6_PORT, GPIO_PIN_NUMBER6 }, GPIO_INPUT };
/*******************************************************************************
 ...
 ******************************************************************************/
bool get_vibration_sense_status(void)
{
    sl_status_t status;
    uint8_t pinvalue = 0;  // Initialize to a default safe value
    status = sl_gpio_driver_get_pin(&sl_gpio_pin_config.port_pin, &pinvalue);

    if (status != SL_STATUS_OK) {
        DEBUGOUT("Pin read failed\n");
        return false;  // Return false if reading the pin fails
    }

    DEBUGOUT("Pin read successful\n");
    return (pinvalue == 1);
}
/*******************************************************************************
