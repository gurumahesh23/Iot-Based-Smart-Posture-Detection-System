 ...
 ******************************************************************************/
void adc_static_mode_example_process_action(float *ph)
{
  sl_status_t status;
  uint16_t adc_value;
  uint8_t channel_num;
  ...
  for (channel_num = 0; channel_num < sl_adc_config.num_of_channel_enable; channel_num++) {
    if (data_sample_complete_flag) {
      ...
      *ph = vout;
      ...
      //DEBUGOUT("ADC Channel[%d] Measured pH value is :%lf\n", channel_num, (double)(pHvalue));
    }
  }
}
void getflexvalue(float *pH)
{
  float buf=0.0f;
  float temph;
  for(int i=0;i<16;i++)
    {
      adc_static_mode_example_process_action(&temph);
      buf+=temph;
    }
  buf=(float)(buf/16);
  *pH = buf;
